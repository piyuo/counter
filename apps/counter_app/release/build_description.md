...
`apps/counter_app/release/build_description.txt`

This file is the English (`en-US`) source.

Keep the original `build_description.txt` unchanged.

Create translated UTF-8 `.txt` files in:

`apps/counter_app/release/description/`

First, copy the English source to:

`apps/counter_app/release/description/en-US.txt`
...
- Translate every file directly from `apps/counter_app/release/build_description.txt`.
...
- ✓ The original `apps/counter_app/release/build_description.txt` was **not** modified.
