...
const _userIdPrefsKey = 'feature_usage_user_id';

NativeUsageBackend? _nativeBackend;
MeasurementProtocolSession? _measurementProtocol;
Object? _initError;

// Usage can be initialized but disabled in case disabling occurs after initialization.
bool _initialized = false;
bool _disabled = true;

/// The native SDK backend (e.g. Firebase Analytics), if one was supplied to
/// [initUsage] and initialized successfully.
NativeUsageBackend? get nativeBackend => _nativeBackend;
...
/// Whether usage initialization was successfully completed.
bool get isUsageInitialized => _initialized;

/// If an error occurred during initialization, it's recorded here rather than thrown.
Object? get usageInitError => _initError;

/// Stop or resume sending events.
bool get isUsageDisabled => _disabled;
set isUsageDisabled(bool value) => _disabled = value;

@visibleForTesting
void setMockNativeBackend(NativeUsageBackend backend) => _nativeBackend = backend;
...
/// Prepares usage for use. Doesn't throw errors; in debug mode failures
/// raise assertions. If initialization fails, [isUsageInitialized] stays false.
///
...
Future<void> initUsage({
  bool sendAppLaunch = true,
  NativeUsageBackend? nativeBackend,
  String? measurementId,
  String? apiSecret,
  String? userId,
}) async {
  if (_initialized) return;
  _disabled = false; // init will enable usage
  try {
    ...
    _initialized = false;
    _initError = e;
    assert(false, "Can't init usage due to error.\n\n$e");
  }
}
